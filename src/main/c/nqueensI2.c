 ...
 * 4.1.2015 implemented symmetrie check. All mirrored versions along the
 ...
 * 4.1.2015 (#1) implement clearQueens inline where necessary.
 *
 *****************************************************************************/
#include <stdio.h>
#include <stdlib.h>
	...
	for (j = 1; j <= outerEnd; j++) {
		...
		while (last) {
			...
			if (x[f.row]) {
				...
				if (x[f.row + 1]) {
					a[x[f.row + 1]] = 1;
					b[f.row + 1 + x[f.row + 1]] = 1;
					c[f.row + 1 - x[f.row + 1] + n] = 1;
					x[f.row + 1] = 0;
				}
			}
		...
		// clear any leftover queens in all rows
		for (y = 1; y < n; y++) {
			if (x[y]) {
				a[x[y]] = 1;
				b[y + x[y]] = 1;
